...
from common.logger import log_eval
...
class EvalCallback(EventCallback):
        ...
        if self.csv_path is not None:
            ...
            if not file_exists or os.path.getsize(self.csv_path) == 0:
                self._csv_writer.writerow(
                    ["total_steps", "mean_reward", "std_reward", "min_reward", "max_reward", "elapsed", "wallclock"]
                )
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and (self.n_calls % self.eval_freq == 0 or self.n_calls == 1):
            ...
            wallclock = time.time() - float(self._training_start_time)

            if self.logger_obj is not None and self.log_context is not None:
                log_eval(self.logger_obj, self.log_context, episode_rewards, elapsed, self.num_timesteps)
            elif self.verbose >= 1:
            ...
            if self._csv_writer is not None:
                self._csv_writer.writerow(
                    [
                        ...
                        f"{float(std_reward):.6f}",
                        f"{float(np.min(episode_rewards)):.6f}",
                        f"{float(np.max(episode_rewards)):.6f}",
                        f"{float(elapsed):.2f}",
                        f"{float(wallclock):.2f}",
                    ]
                )
                self._csv_file.flush()
            ...
            if self.on_eval_complete is not None:
                self.on_eval_complete(self.num_timesteps)
