...
def _tanh_log_det_jacobian(x):
    y = jnp.tanh(x)
    return jnp.log(jnp.clip(1.0 - jnp.square(y), 1e-12))
...
def single_sample(seed, model_state, params, obs, integrator, diffusion_model, stop_grad=False):
    ...
    running_cost = -(log_ratio + _tanh_log_det_jacobian(final_x).sum())
    # running_cost = -log_ratio

    final_x = jnp.tanh(final_x)
    x_t = per_step_output
