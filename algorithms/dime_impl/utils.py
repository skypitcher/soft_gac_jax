...
def log_prob_kernel(x, mean, scale):
    log_unnormalized = -0.5 * jnp.square((x - mean) / scale)
...
def init_dt(cfg):
    if cfg.sampler.per_step_dt:
        dt_schedule = cfg.sampler.dt_schedule
        return inverse_softplus(jnp.ones(cfg.alg.actor.diff_steps) * cfg.sampler.dt * dt_schedule(jnp.arange(cfg.alg.actor.diff_steps)))
    else:
        return jnp.ones(1) * inverse_softplus(cfg.sampler.dt)


def get_sampler_init(alg_name):

    if alg_name == 'dis':
        from algorithms.dime_impl.od.dis import init_dis
        return init_dis
